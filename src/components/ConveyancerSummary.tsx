...
export default function ConveyancerSummary({ sections, propertyAddress, sellerName }: ConveyancerSummaryProps) {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 pb-8">
      {/* Header */}
      <h2 className="text-xl font-bold text-gray-900 mb-1">Conveyancer Summary</h2>
      <p className="text-sm text-gray-500 mb-5">Risk analysis based on form responses</p>

      {/* Overview cards */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="rounded-xl border-2 border-red-200 bg-red-50 p-3 text-center">
          <div className="text-2xl font-bold text-red-700">{summary.redFlags.length}</div>
          <div className="text-xs font-semibold text-red-600 uppercase">Red</div>
        </div>
        <div className="rounded-xl border-2 border-amber-200 bg-amber-50 p-3 text-center">
          <div className="text-2xl font-bold text-amber-700">{summary.amberFlags.length}</div>
          <div className="text-xs font-semibold text-amber-600 uppercase">Amber</div>
        </div>
        <div className="rounded-xl border-2 border-green-200 bg-green-50 p-3 text-center">
          <div className="text-2xl font-bold text-green-700">{summary.greenCount}</div>
          <div className="text-xs font-semibold text-green-600 uppercase">Clear</div>
        </div>
      </div>
      ...
      {summary.redFlags.length > 0 && (
        <div className="mb-4">
          <button
            ...
            className="w-full flex items-center justify-between rounded-xl border-2 border-red-200 bg-red-50 px-4 py-3 text-left"
          >
      ...
      {summary.amberFlags.length > 0 && (
        <div className="mb-4">
          <button
            ...
            className="w-full flex items-center justify-between rounded-xl border-2 border-amber-200 bg-amber-50 px-4 py-3 text-left"
          >
      ...
      {/* Green summary */}
      <div className="rounded-xl border-2 border-green-200 bg-green-50 px-4 py-3 mb-6">
      ...
      <button
        ...
        className="w-full rounded-xl bg-gray-900 py-3.5 text-base font-semibold text-white shadow-sm transition-colors active:bg-gray-800 disabled:opacity-50 flex items-center justify-center gap-2"
      >
...
function FlagCard({ flag }: { flag: RiskFlag }) {
  ...
  return (
    <div className={`rounded-xl border px-4 py-3 ${
      isRed ? 'border-red-200 bg-white' : 'border-amber-200 bg-white'
    }`}>
      <div className="flex items-start gap-2 mb-1">
        ...
        <div>
          <div className="text-xs font-medium text-gray-500">{flag.section}</div>
          <div className="text-sm font-semibold text-gray-900">{flag.summary}</div>
        </div>
      </div>
      <div className={`mt-2 rounded-lg px-3 py-2 text-xs ${
        isRed ? 'bg-red-50 text-red-800' : 'bg-amber-50 text-amber-800'
      }`}>
