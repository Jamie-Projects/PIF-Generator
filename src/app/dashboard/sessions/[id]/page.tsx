...
export default function SessionDetailPage() {
  ...
  if (!session) {
    return <div className="text-center py-8 text-gray-500">Session not found</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm mb-6">
        <h3 className="text-sm font-medium text-gray-700 mb-2">Client Form Link</h3>
        <div className="flex gap-2">
          <input
            readOnly
      ...
      <div className="space-y-3">
        <h3 className="text-lg font-bold text-gray-900">Section Data</h3>
        {session.propertyForm?.sections.map((section) => {
          ...
          return (
            <details key={section.sectionKey} className="rounded-xl bg-white shadow-sm">
              <summary className="flex cursor-pointer items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-400">Part {sectionDef?.part}</span>
                </div>
                <span
                    ...
                    {sectionDef?.fields.map((field) => {
                      ...
                      return (
                        <div key={field.key} className="flex flex-col sm:flex-row sm:gap-4">
                          <dt className="text-sm font-medium text-gray-500 sm:w-1/3">{field.label}</dt>
